...
export const DEFAULT_FILM: LandingFilm = {
	...
	testimonials: [{"quote":"HostOS Collective completely transformed how we handle online orders. Our operations are smoother, and we’ve seen a 30% boost in repeat customers since launching the new system.","name":"Zack Holmes","role":"Restaurant owner"},{"quote":"From building our website to setting up automated email campaigns, HostOS Collective delivered everything on time and on budget. Truly a one-stop shop for growing businesses.","name":"Miguel Chavez","role":"E-commerce startup"},{"quote":"Having a dedicated VA team that manages my fleet bookings and guest communication has been a game changer. Professional, reliable, and always responsive.","name":"Matt Tolley","role":"Turo host"}],
...
export function normalizeFilm(raw: unknown): LandingFilm {
	...
	const testimonials = tRaw
		? tRaw.slice(0, 8).map((t) => { const x = (t && typeof t === "object" ? t : {}) as Record<string, unknown>; return { quote: str(x.quote, "", 400), name: str(x.name, "", 60), role: str(x.role, "", 60) }; }).filter((t) => t.quote && t.name)
