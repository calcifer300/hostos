...
import { canManageSettings, getAccessibleModules, getChosenVertical, getCurrentHostId } from "@/lib/host/context";
...
export default async function StartPage() {
  ...
  const [host, workspaceModules, modules, canEnable] = await Promise.all([getHost(hostId), getHostModules(hostId), getAccessibleModules(), canManageSettings()]);
  ...
  return <VerticalChooser firstName={firstName} enabled={modules} workspaceEnabled={workspaceModules} current={current} canEnable={canEnable} workspaceName={host?.name ?? "Your workspace"} />;
}
