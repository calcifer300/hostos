...
export function Portrait({ member, sizes = GRID_SIZES, fallbackSize = 40, priority = false }: { member: TeamProfile; sizes?: string; fallbackSize?: number; priority?: boolean }) {
  ...
  if (src && isOptimizableSrc(src)) {
    return <Image src={src} alt={member.name} fill sizes={sizes} priority={priority} className="scale-[1.12] object-cover" style={{ objectPosition: member.photoFocus || "50% 15%" }} onError={fail} />;
  }
  if (src) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={src} alt={member.name} className="absolute inset-0 h-full w-full scale-[1.12] object-cover" style={{ objectPosition: member.photoFocus || "50% 15%" }} onError={fail} />;
  }
  return (
