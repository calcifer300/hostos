...
<Section id="platform" eyebrow={PLATFORM.eyebrow} title={PLATFORM.title} lede={PLATFORM.lede}>
	<div use:reveal class="overflow-hidden rounded-3xl border border-line bg-surface-1 shadow-2">
		<div class="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3">
			...
			<div class="flex rounded-full border border-line bg-bg p-1" role="tablist" aria-label="Screens" tabindex="-1" onkeydown={onKey}>
				{#each PLATFORM.tabs as t, i}
					<button type="button" role="tab" aria-selected={tab === i} aria-controls={`demo-${t.id}`} tabindex={tab === i ? 0 : -1} onclick={() => (tab = i)} class={`rounded-full px-4 py-1.5 text-[13px] font-medium transition-colors ${tab === i ? 'bg-surface-3 text-ink' : 'text-ink-3 hover:text-ink'}`}>{t.name}</button>
				{/each}
			</div>
		</div>
		{#key tab}
				...
				{#each current.widgets as w, i}
					{#if w.kind === 'kpi'}
						<div class="reveal is-in rounded-2xl border border-line bg-surface-2 p-4" style={`--reveal-delay:${i * 60}ms`}>
							...
							<p class="mt-2 text-[30px] font-semibold leading-none tracking-tight text-ink">{w.value}</p>
