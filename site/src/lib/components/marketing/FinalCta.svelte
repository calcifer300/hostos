	...
	<div class="container-x grid items-start gap-12 lg:grid-cols-[1fr_1fr]">
		<div use:reveal>
			...
			<a href={contact.facebookUrl} target="_blank" rel="noopener" class="mt-8 flex items-center gap-4 rounded-2xl border border-line bg-surface-2/80 p-4 transition-[border-color,transform] duration-300 hover:-translate-y-0.5 hover:border-accent/60">
				<span class="relative h-12 w-12 shrink-0"><img src={founderPhoto} alt="" width="48" height="48" loading="lazy" class="h-12 w-12 rounded-full border-2 border-surface-3 object-cover" style="object-position:50% 25%" /><span class="absolute -bottom-1 -right-1 grid h-5 w-5 place-items-center rounded-full bg-[#1877f2] text-white ring-2 ring-surface-2"><svg viewBox="0 0 24 24" class="h-3 w-3" fill="currentColor" aria-hidden="true"><path d="M13.5 22v-8h2.7l.4-3.2h-3.1V8.8c0-.9.3-1.6 1.6-1.6h1.7V4.4c-.3 0-1.3-.1-2.5-.1-2.5 0-4.1 1.5-4.1 4.2v2.3H7.4V14h2.8v8z" /></svg></span></span>
